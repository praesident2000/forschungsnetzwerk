...
function listable_child_enqueue_styles() {
	...
	wp_enqueue_style( 'cf-custom',
		get_stylesheet_directory_uri() . '/assets/css/cf-custom.css',
		array(), //make sure the the child's style.css comes after the parents so you can overwrite rules
		100,
		true
	);
	wp_enqueue_script( 'script', get_stylesheet_directory_uri() . '/js/scripts.js', array('jquery','listable-scripts','cf-forms'), 101, true );
	wp_enqueue_script( 'filters', get_stylesheet_directory_uri() . '/js/filters.js', array('jquery','listable-scripts','wp-job-manager-ajax-filters'), 111, true );
}
